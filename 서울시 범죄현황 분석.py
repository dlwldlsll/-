...
crime_by_district = crime_data[['자치구별(1)', '자치구별(2)', '2022', '2022.1']]

# '발생', '검거' 열의 데이터 타입을 숫자로 변환
crime_by_district['2022'] = pd.to_numeric(crime_by_district['2022'], errors='coerce')
crime_by_district['2022.1'] = pd.to_numeric(crime_by_district['2022.1'], errors='coerce')

# 열 이름 변경
crime_by_district.columns = ['자치구', '범죄유형', '발생', '검거']

# '범죄유형', '발생', '검거' 열을 기준으로 피벗 테이블 생성
crime_pivot = crime_by_district.pivot_table(index=['자치구', '범죄유형'])
...
plt.title('서울시 구별 범죄 발생 및 검거 현황')
plt.xlabel('자치구, 범죄유형')
plt.ylabel('건수')
